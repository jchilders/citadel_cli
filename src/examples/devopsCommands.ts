...
import { JsonCommandResult, TextCommandResult } from '../components/Citadel/types/command-results';
...
export function createDevOpsCommandRegistry(): CommandRegistry {
  ...
  commandRegistry.addCommand(
    [
      { type: 'word', name: 'deploy' },
      { type: 'word', name: 'production' },
      { type: 'argument', name: 'version', description: 'Version to deploy (e.g., 1.2.3)' }
    ],
  ...
  commandRegistry.addCommand(
    [
      { type: 'word', name: 'infra' },
      { type: 'word', name: 'scale' },
      { type: 'argument', name: 'instances', description: 'Number of instances' }
    ],
    'Scale infrastructure resources',
    async (args: string[]) => {
      await new Promise(resolve => setTimeout(resolve, 1200));
      const instances = parseInt(args[0], 10);
      return new JsonCommandResult({
        service: 'web-server',
